...
export default function Navbar() {
  ...
  const isSpecialMode = isP2P || isDistributor

  const navLinks = isP2P
    ? [{ path: '/p2p', label: 'P2P Intelligence' }]
    : isDistributor
      ? [{ path: '/distributor', label: 'Vendor Recruitment' }]
      : [
          { path: '/', label: 'Home' },
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isP2P ? 'p2p-mode' : isDistributor ? 'distributor-mode' : ''} ${
        scrolled
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-16 relative">
          ...
          <Link to={isP2P ? '/p2p' : isDistributor ? '/distributor' : '/'} className="flex items-center gap-3 group flex-shrink-0">
